...
void findSCCs(int size,IntList*  adjVertices,int* color, int* dtime,int* ftime, int* parent){
  printf("dfsPhase1:\n");
  dfsSweepInt(size, adjVertices,color, dtime, ftime, parent);
  printDfsTrace(size,color, dtime, ftime, parent);
  int n = size-1;
  printf("\nTranspose:\n");
  adjVertices = transposeGraph(adjVertices,size);
...
int main(int argc, char * argv[])
{
  ...
  */
	  ...
	  findSCCs(size,adjVertices,color, dtime,ftime,parent);
	  exit(0);
  }
