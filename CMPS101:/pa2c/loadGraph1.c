 ...
 */
 ...
 void loadEdgeList(int size,EdgeList* adjVertices, char* file)
 {
	...
	while (fgetsRetn == line){
		Edge e = parseEdge(line);
		adjVertices[e.from] = edgeCons(e.to, e.wgt, adjVertices[e.from]);
		fgetsRetn = fgets(line, 1024, inbuf);
	}
...
void printIntList(int size, IntList* adjVertices)
{
  ...
  //Go through all the vertices
    for(int i = 1;size > i;++i){
      ...
      while(next != intNil){
	element = firstInt(next);
	fprintf(stdin," %d,",element); 
	next = intRest(next);
      }
...
void printEdgeList(int size, IntList* adjVertices)
{
  EdgeList next;
  EdgeInfo element;
  ...
  //Go through all the vertices
    for(int i = 1;size > i;++i){
      ...
      // Print all the edges from this Vertex
      while(next != intNil){
	element = firsEdge(next);
	fprintf(stdin," %d %lf,",element.to, element.wgt); 
	next = intRest(next);
      }
