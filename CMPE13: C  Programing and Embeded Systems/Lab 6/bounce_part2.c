...
AdcResult eventInterrupt = {false, 0};
static double currentAdcValue = 0;
...
int main(void)
{
    ...
    while (1) {
        // ADC Average
        currentAdcValue = (ADC1BUF0 + ADC1BUF1 + ADC1BUF2 + ADC1BUF3 +
                ADC1BUF4 + ADC1BUF5 + ADC1BUF6 + ADC1BUF7) / 8;

        // Calculate the percentage
        percentage = (currentAdcValue / 1023)*100;

        // Check for event to update the screen
        if (eventInterrupt.event) {
            sprintf(buffer, "Potentiometer Value:\n%4.0f\n%3.0f%%", currentAdcValue, percentage);
...
void __ISR(_ADC_VECTOR, ipl2auto)AdcHandler(void)
{
    ...
    if (currentAdcValue != eventInterrupt.value) {
        eventInterrupt.event = true;
        eventInterrupt.value = currentAdcValue;
    }
}