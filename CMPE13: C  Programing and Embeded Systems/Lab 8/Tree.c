...
{
    Node *nodeChild = malloc(sizeof (Node));
    if (nodeChild) {
        if (level > 0) {
            nodeChild->data = data[level];
            nodeChild->leftChild = TreeCreate(level - 1, data);

        } else {
            nodeChild->data = data[level];
            nodeChild->leftChild = NULL;
            nodeChild->rightChild = NULL;

        }
        if (level > 0) {
            nodeChild->data = data[level];
            nodeChild->rightChild = TreeCreate(level - 1, data);
        } else {
            nodeChild->data = data[level];
            nodeChild->leftChild = NULL;
            nodeChild->rightChild = NULL;
